...
async function callTogether(systemPrompt, userMessage, options = {}) {
  ...
  try {
    response = await together.chat.completions.create({
      ...
      messages: [
        { role: 'system', content: options.rawText ? systemPrompt : systemPrompt + '\n\nIMPORTANT: Return ONLY valid JSON. No markdown, no explanation.' },
  ...
  if (options.rawText) {
    return {
      data: text.trim(),
      meta: { provider: 'together', model, tokens_used: { input: response.usage?.prompt_tokens || 0, output: response.usage?.completion_tokens || 0, cached: 0 }, duration_ms: Date.now() - startTime }
    }
  }
